...
function RouteComponent() {
  const [data, setData] =
    useState<Paths.DashboardReportsTotalChats.Responses.$200>();
  ...
  const getTotalChats = async (
    ...
    try {
      ...
      const { data } = await client.DashboardReportsTotalChats(
        {
      ...
      setData(data);
      const { data: heatMapData } = await client.DashboardReportsTotalChats(
        {
          app_slug: import.meta.env.VITE_APP_SLUG,
        },
        {
  ...
  return (
    <div className="flex flex-col w-full p-4 max-w-[1200px]">
      <div className="w-full border-b border-gray-200 mb-4">
        <div className="p-4 text-lg font-semibold">Total Chats Report</div>
      </div>
      {/* FILTERS */}
