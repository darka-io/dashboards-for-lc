...
function RouteComponent() {
  const [data, setData] =
    useState<Paths.DashboardReportsChatsSatisfaction.Responses.$200>();
  const [loading, setLoading] = useState(false);
  const { selectedGroup, agents, tags, groups, userFilters } =
    useContext(ReportContext);
  const getTotalChats = async (
    props: Paths.DashboardReportsChatsSatisfaction.RequestBody
  ) => {
    setLoading(true);
    try {
  ...
  const [filters, setFilters] =
    useState<Paths.DashboardReportsChatsSatisfaction.RequestBody>({
