...
function RouteComponent() {
  const [data, setData] =
    useState<Paths.DashboardReportsChatDuration.Responses.$200>();
  const [loading, setLoading] = useState(false);
  const { selectedGroup, agents, tags, groups, userFilters } =
    useContext(ReportContext);
  const getTotalChats = async (
    props: Paths.DashboardReportsChatDuration.RequestBody
  ) => {
    setLoading(true);
    try {
  ...
  const [filters, setFilters] =
    useState<Paths.DashboardReportsChatDuration.RequestBody>({
            ...
            series={[
              {
                ...
                data: Object.keys(data?.records ?? {}).map((k) => ({
                  ...
                  y:
                    Math.ceil(((data?.records?.[k].duration ?? 0) / 60) * 100) /
                    100,
                })),
              },
              {
                ...
                data: Object.keys(data?.records ?? {}).map((k) => ({
                  ...
                  y:
                    Math.ceil(
                      ((data?.records?.[k].agents_chatting_duration ?? 0) /
                        60) *
                        100
                    ) / 100,
                })),
