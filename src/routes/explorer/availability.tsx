...
function RouteComponent() {
    const [data, setData] = useState<Paths.DashboardReportsChatAvailability.Responses.$200>()
    const [loading, setLoading] = useState(false)
    const { selectedGroup, agents, tags, groups } = useContext(ReportContext)
    const getTotalChats = async (props: Paths.DashboardReportsChatAvailability.RequestBody) => {
        setLoading(true)
        try {
            const client = await API()
            const { data } = await client.DashboardReportsChatAvailability({
    ...
    const [filters, setFilters] = useState<Paths.DashboardReportsChatAvailability.RequestBody>({
    ...
    return <div className='flex flex-col w-full p-4 max-w-[1200px]'>
        <div className='w-full border-b border-gray-200 mb-4'>
            <div className='p-4 text-lg font-semibold'>Chat Availability Report</div>
        </div>
        {/* FILTERS */}
