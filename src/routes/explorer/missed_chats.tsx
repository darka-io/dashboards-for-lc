...
function RouteComponent() {
  const [data, setData] =
    useState<Paths.DashboardReportsMissedChats.Responses.$200>();
  ...
  const getTotalChats = async (
    props: Paths.DashboardReportsMissedChats.RequestBody
  ) => {
    setLoading(true);
    try {
  ...
  const [filters, setFilters] =
    useState<Paths.DashboardReportsMissedChats.RequestBody>({
  ...
  return (
    <div className="flex flex-col w-full p-4 max-w-[1200px]">
      ...
      <Spin spinning={loading}>
        ...
        <HeatMap data={(heatMapData?.records ?? []) as never} />
      </Spin>
    </div>
  );
}
