...
function RouteComponent() {
  const [filters, setFilters] =
    useState<Paths.DashboardReportsAgentPerformance.RequestBody>({
  ...
  const [data, setData] =
    useState<Paths.DashboardReportsAgentPerformance.Responses.$200>();
  const [loading, setLoading] = useState(false);
  const getPerformanceReport = async (
    props: Paths.DashboardReportsAgentPerformance.RequestBody
  ) => {
    setLoading(true);
    try {
  ...
  const selectedGroupIds = filters.groups?.length
    ? filters.groups
  ...
  const filteredAgents = agents.filter((agent) => {
    if (!agent.groups?.length) return false;
    return agent.groups.some((g) => selectedGroupIds.includes(g.id));
  });
      ...
      <Spin spinning={loading}>
        <div className="flex flex-col   gap-2 min-w-[1000px] mt-6 overlow-y-auto">
          ...
          <div className="flex itmes-stretch gap-4 justify-start w-full w-[100px]">
            <div className=" text-gray-400 text-[14px] w-[300px]">Agent</div>
            ...
            return (
              <div className="flex flex-row gap-4 items-center justify-start w-full">
                ...
                <div className={TableItemStyle}>
                  {data?.records[a.id]?.chats_count ?? 0}
                </div>
                {/* SATISFACTION */}
                <div className={TableItemStyle2}>
                  {(
                    Math.ceil(
                      ((data?.records[a.id]?.chats_rated_good ?? 0) * 100) /
                        (data?.records[a.id]?.chats_count ?? 1)
                    ) / 100
                  ).toFixed(2)}
                </div>
                {/* FIRST RESPONSE */}
                <div className={TableItemStyle}>
                  {dayjs
                    .duration(
                      (data?.records[a.id]?.first_response_time ?? 0) * 1000
                    )
                    .format("HH:mm:ss")}
                </div>
                {/* CHATTING TIME */}
                <div className={TableItemStyle2}>
                  {dayjs
                    .duration((data?.records[a.id]?.chatting_time ?? 0) * 1000)
                    .format("HH:mm:ss")}
                </div>
                {/* ACCEPTING TIME */}
                <div className={TableItemStyle}>
                  {dayjs
                    .duration(
                      (data?.records[a.id]?.accepting_chats_time ?? 0) * 1000
                    )
                    .format("HH:mm:ss")}
                </div>
                {/* Efiiciency */}
                <div className={TableItemStyle2}>
                  {data?.records[a.id]?.accepting_chats_time &&
                  data?.records[a.id]?.chats_count
                    ? (
                        Math.ceil(
                          ((data?.records[a.id]?.chats_count ?? 1) * 100) /
                            ((data?.records[a.id]?.accepting_chats_time ?? 0) /
                              3600)
                        ) / 100
                      ).toFixed(2)
