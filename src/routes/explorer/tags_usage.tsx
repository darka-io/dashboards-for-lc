...
function RouteComponent() {
  const [data, setData] =
    useState<Paths.DashboardReportsTagsUsage.Responses.$200>();
  const [dataWithoutTags, setDataWithoutTags] =
    useState<Paths.DashboardReportsTagsUsage.Responses.$200>();
  const [loading, setLoading] = useState(false);
  const { selectedGroup, agents, tags, groups, userFilters } =
    useContext(ReportContext);
  const getTotalChats = async (
    props: Paths.DashboardReportsTagsUsage.RequestBody
  ) => {
    setLoading(true);
    try {
  ...
  const [filters, setFilters] =
    useState<Paths.DashboardReportsTagsUsage.RequestBody>({
